...
for (let i = 0; i < starWarsCharacters.length; i++) {
  let names = starWarsCharacters[i].name;
...
for(let i = 0; i< femaleCharacters.length; i++){
  let nameF = femaleCharacters[i].name;

  for(let n = 0; n < charactersNames.length; n++){

    if(nameF === charactersNames[n]){
      charactersNames.splice(n, 1);
    };
  };
};
console.log(charactersNames);

// correzione - altro metodo

for(let i=0; i < charactersNames.length; i++){
  const characterStr = charactersNames[i];

  for(let n = 0; n < femaleCharacters.length; n++){
    const femCharObj = femaleCharacters[n];
...
console.log("il personaggio", randomlySelectedCharacter.name, "è alto", randomlySelectedCharacter.height, "con i capelli di colore", randomlySelectedCharacter.hair_color, "...");